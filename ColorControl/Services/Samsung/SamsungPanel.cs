    ...
    partial class SamsungPanel : UserControl, IModulePanel
    {
        ...
        private void SetSamsungDevicePowerOptions()
        {
            ...
            try
            {
                ...
                clbLgPower.SetItemChecked(10, device?.Options.PowerOffAfterSessionLock ?? false);
                clbLgPower.SetItemChecked(11, device?.Options.PowerOnAfterSessionUnlock ?? false);
                clbLgPower.SetItemChecked(12, device?.Options.UseSecureConnection ?? false);
            }
            finally
            {
        ...
        private void clbLgPower_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            if (e.Index is 0 or 1 or 5 or 8 && !(device.Options.PowerOnAfterResume || device.Options.PowerOnAfterStartup || device.Options.PowerOnAfterScreenSaver || device.Options.PowerOnByWindows || device.Options.PowerOnAfterSessionUnlock))
            {
                MessageForms.InfoOk(
...
Use 'Settings > Test power off/on' to test this functionality."
            ...
            FormUtils.BeginInvokeCheck(this, () =>
            {
                ...
                device.Options.PowerOffAfterSessionLock = clbLgPower.GetItemChecked(10);
                device.Options.PowerOnAfterSessionUnlock = clbLgPower.GetItemChecked(11);
                device.Options.UseSecureConnection = clbLgPower.GetItemChecked(12);
