...
public class ADLWrapper
{
	...
	private static ADL_CONTEXT_HANDLE context = ADL_CONTEXT_HANDLE.Zero;
	...
	public static void Uninitialze()
	{
		...
		if (null != del2 && context != ADL_CONTEXT_HANDLE.Zero)
		{
			del2(context);
		}
	}
	...
	public static ADLAdapterInfoArray? GetAdapters()
	{
		...
		if (NumberOfAdapters > 0)
		{
			...
			{
				ADL_CONTEXT_HANDLE AdapterBuffer = ADL_CONTEXT_HANDLE.Zero;
				...
				if (ADL_CONTEXT_HANDLE.Zero != AdapterBuffer)
				{
	...
	public static List<ADLDisplayInfo> GetAllDisplays(bool connected = true)
	{
		...
		if (null != del)
		{
			ADL_CONTEXT_HANDLE DisplayBuffer = ADL_CONTEXT_HANDLE.Zero;
			int j = 0;
			...
			if (ADL.ADL_SUCCESS == ADLRet)
			{
				...
				{
					oneDisplayInfo = (ADLDisplayInfo)Marshal.PtrToStructure(new ADL_CONTEXT_HANDLE(DisplayBuffer.ToInt64() + j * Marshal.SizeOf(oneDisplayInfo)), oneDisplayInfo.GetType());
			...
			if (ADL_CONTEXT_HANDLE.Zero != DisplayBuffer)
