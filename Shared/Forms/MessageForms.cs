...
namespace ColorControl.Shared.Forms
{
	...
	public class MessageForms
	{
		...
		public static T SelectObjectValue<T>(string currentValue, string label, IEnumerable<T> values)
		{
			var field = new FieldDefinition
			{
				Label = label,
				FieldType = FieldType.DropDown,
				Values = values.Select(v => v.ToString()),
				Value = currentValue
			};
			...
			return values.First(v => v.ToString() == strValue);
		}
	}
}